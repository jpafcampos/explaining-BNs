...
def assign_bucket(sdp_value, buckets, tolerance=0.05):
    """
    Assign an SDP value to the closest bucket. Returns the bucket value
    if the SDP falls within `tolerance` of it, otherwise returns the
    nearest bucket regardless (so every patient is recorded). Set
    tolerance to None to always snap to the nearest bucket.
    """
    if sdp_value is None:
        return None
    nearest = min(buckets, key=lambda b: abs(sdp_value - b))
    if tolerance is None or abs(sdp_value - nearest) <= tolerance:
        return nearest
    return nearest
...
def generate_random_patients(bn, target_node, target_value, decision_threshold,
    ...
    Generate N random patients (random evidence assignments) that:
      - pass the memory wall (tensor size within budget for both the
        SDP partitions and the VE estimate)
      - have a base posterior meeting the decision threshold

    For each accepted patient, computes its exact SDP and assigns it to the
    nearest bucket. No bucket-filling logic: we simply collect N patients.

    Returns
    -------
    list of dicts, each:
        {'patient', 'exact_sdp', 'bucket', 'partitions'}
    plus diagnostic counters in a second returned dict.
    """
    ...
    below_threshold = 0
    attempts = 0
    max_attempts = n_patients * max_attempts_multiplier

    print(f"\nGenerating {n_patients} random patients "
          f"(Locking {n_evidence} variables as evidence)")
    print(f"Memory wall: tensor size ≤ {max_tensor_entries:,} entries "
          f"(~{max_tensor_entries * 8 / 1024**3:.1f} GB raw)")

    while len(collected) < n_patients and attempts < max_attempts:
        attempts += 1
        ...
        if not sdp_ok or exact_sdp is None:
            sdp_failures += 1
        ...
        # 6. Assign to nearest bucket and store
        bucket = assign_bucket(exact_sdp, buckets)
        collected.append({
            'patient': temp_patient.copy(),
            'exact_sdp': exact_sdp,
            'bucket': bucket,
            'partitions': partitions,
        })
        print(f"--> Patient {len(collected)}/{n_patients}: Exact SDP {exact_sdp:.4f} "
              f"-> bucket {bucket} (tensor size: {max_tensor:,})")

    # Summary
    print(f"\nGeneration summary:")
    print(f"  Patients collected:  {len(collected)}/{n_patients}")
    ...
    if len(collected) < n_patients:
        print(f"  -> Only collected {len(collected)} patients after "
...
def process_single_file(args):
    ...
    file, H_RATIOS, DECISION_THRESHOLD, SIZES_TO_RUN, DENSITIES_TO_RUN, MCMC_TRIALS, N_PATIENTS = args
    results = []
    process = psutil.Process(os.getpid())
    ...
    for H_RATIO in H_RATIOS:
        ...
        gc.collect()  # Clean up before generation, which can be memory-intensive

        # Generate N random patients instead of harvesting per-bucket
        collected, diagnostics = generate_random_patients(
        ...
        # Now benchmark every collected patient
        for entry in collected:
            patient = entry['patient']
            exact_sdp_seed = entry['exact_sdp']
            target_sdp = entry['bucket']  # the assigned bucket, kept for the CSV column

            hidden_vars = [n for n in bn.nodes() if n not in patient and n != target]
            print(f"\n  -> Benchmarking patient for bucket {target_sdp} (Exact: {exact_sdp_seed:.4f})")
            ...
            exact_sdp, exact_time, exact_success = run_for_time(
                fast_broadcast_sdp, bn, target, target_value, patient, DECISION_THRESHOLD, partitions, timeout_sec=120
            )
            ...
            results.append({
                ...
                'Exact_SDP_Status': 'OK' if exact_success else 'FAILED',
...
def run_targeted_sdp_experiment(bif_directory, output_csv="targeted_sdp_random_bns.csv", n_workers=4, n_patients=5):
    ...
    args_list = [
        (file, H_RATIOS, DECISION_THRESHOLD, SIZES_TO_RUN, DENSITIES_TO_RUN, MCMC_TRIALS, n_patients)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='results/random_experiment_75.csv')
    parser.add_argument('--n-workers', type=int, default=1)
    parser.add_argument('--n-patients', type=int, default=7,
                        help='Number of random patients to generate and benchmark per network/H-ratio')
    args = parser.parse_args()

    os.makedirs(os.path.dirname(args.output), exist_ok=True)
    run_targeted_sdp_experiment(args.bif_dir, args.output, args.n_workers, args.n_patients)